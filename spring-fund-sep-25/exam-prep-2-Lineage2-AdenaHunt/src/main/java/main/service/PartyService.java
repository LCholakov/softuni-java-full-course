...
public class PartyService {
    ...
    public void inviteToParty(UUID senderId, UUID receiverId) {
        ...
        if(receiver.getParty() != null) {
            throw new DomainException("Player " + receiver.getNickname() + " is already in a party.");
        }

        Party party = getParty(sender);

        List<Player> allMembersUntillNow = playerService.findAllByParty(party);
        if(allMembersUntillNow.size() >= 3) {
            throw new DomainException("Party is full. Cannot invite more players.");
        }
    ...
    public void dismissParty(UUID partyId) {
        Party party = partyRepository.findById(partyId).orElseThrow(() -> new DomainException("Party with id " + partyId + " is not found."));
