...
public class WalletService {
    private static final String SMART_WALLET_IDENTIFIER = "SMART WALLET";
    ...
    private static final String WALLET_NOT_FOUND_FAILURE_REASON = "You don't own this wallet";
    ...
    @Transactional
    public Transaction withdraw(User user, UUID walletId, BigDecimal amount, String description) {
        ...
        if (!isActiveWallet(wallet)) {
            ...
            transaction.setFailureReason(WALLET_NOT_FOUND_FAILURE_REASON);
            transaction.setStatus(TransactionStatus.FAILED);
        }else { // successfull transaction
    ...
    public boolean isActiveWallet(Wallet wallet) {
        return wallet.getStatus().equals(WalletStatus.ACTIVE);
    }
...
//    private methods below all public
    ...
    public Transaction transfer(TransferRequest transferRequest) {
        ...
        String description = TRANSFER_DESCRIPTION_FORMAT.formatted(senderWallet.getOwner().getUsername()
                , receiverWallet.getOwner().getUsername(), transferRequest.getAmount());
        Transaction withdrawalTransaction = withdraw(senderWallet.getOwner(), senderWallet.getId(), transferRequest.getAmount(), description);
        if(withdrawalTransaction.getStatus() == TransactionStatus.SUCCEEDED) {
            Transaction depositTransaction = deposit(receiverWallet.getId(), transferRequest.getAmount(), description);
        }
